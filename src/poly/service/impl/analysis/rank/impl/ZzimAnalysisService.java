...
@Service("ZzimAnalysisService")
public class ZzimAnalysisService extends MongoDBComon implements IZzimAnalysisService, ICommCont {
	...
	private int doAnalysisStep1() throws Exception {
		...
		try {
			...
			String nColNm = "QUIZRANK_ZZIM_ANALYSIS_" + ContAnaysisStdDay + "_STEP1";
			...
			List<? extends Bson> pipeline = Arrays.asList(
					new Document().append("$group",
							new Document().append("_id", new Document())
									.append("MAX(zzim_cnt)", new Document().append("$max", "$zzim_cnt"))
									.append("AVG(zzim_cnt)", new Document().append("$avg", "$zzim_cnt"))),
					new Document().append("$project", new Document().append("max_cnt", "$MAX(zzim_cnt)")
							.append("avg_cnt", "$AVG(zzim_cnt)").append("_id", 0)));
	...
	private int doAnalysisStep2() throws Exception {
		...
		try {
			...
			// 컬렉션 이름
			String nColNm = "QUIZRANK_ZZIM_ANALYSIS_" + ContAnaysisStdDay + "_STEP2";
			...
			while (cursor.hasNext()) {
				...
				// 퀴즈팩별 중복되지 않는 좋아요수
				long cnt = (long) current.get("zzim_cnt");
				doc.append("cnt", cnt);
	 ...
	 * 분석 3단계
	 * 
	 * 최근 7일동안의 일자별 퀴즈팩들의 찜 및 가중치 적용된 찜률 데이터 구조 생성
	 */
	private int doAnalysisStep3() throws Exception {
		...
		try {
			...
			String nColNm = "QUIZRANK_ZZIM_ANALYSIS_" + ContAnaysisStdDay + "_STEP3";
			...
			/*
			 * #############################################################################
			 * 최근 7일동안의 일자별 퀴즈팩들의 찜 및 가중치 적용된 찜 데이터 구조 생성 시작!!
			 * #############################################################################
			 */
			...
			for (int i = 0; i < ContReadCntDay; i++) {
				...
				// 이전 좋아요수 저장 컬렉션 로드하기
				MongoCollection<Document> rCol = mongodb.getCollection("QUIZRANK_ZZIM_ANALYSIS_" + stdDay + "_STEP2");

				log.info("#################################################################");
				log.info("# Read QUIZRANK_ZZIM_ANALYSIS_" + stdDay + "_STEP2 Analysis Start!!");
				log.info("#################################################################");
				...
				log.info("#################################################################");
				log.info("# Save QUIZRANK_ZZIM_ANALYSIS_" + stdDay + "_STEP3 Analysis Result!!");
				log.info("# " + nColNm + " insert Doc Count : " + mongodb.getCollection(nColNm).countDocuments());
				log.info("#################################################################");
			...
			/*
			 * #############################################################################
			 * 최근 7일동안의 일자별 퀴즈팩들의 찜 및 가중치 적용된 찜 데이터 구조 생성 끝!!
			 * #############################################################################
			 */
	...
	private int doAnalysisStep4() throws Exception {
		...
		try {
			...
			String nColNm = "QUIZRANK_ZZIM_ANALYSIS_" + ContAnaysisStdDay + "_STEP4";
			...
			MongoCollection<Document> rCol = mongodb
					.getCollection("QUIZRANK_ZZIM_ANALYSIS_" + ContAnaysisStdDay + "_STEP3");
	...
	private int doCleanData() throws Exception {
		...
		try {
			...
			// 1단계 분석 삭제
			dropColNm = "QUIZRANK_ZZIM_ANALYSIS_" + cleanDay + "_STEP1";
			...
			// 3단계 분석 삭제
			dropColNm = "QUIZRANK_ZZIM_ANALYSIS_" + cleanDay + "_STEP3";
			...
			// 4단계 분석 삭제
			dropColNm = "QUIZRANK_ZZIM_ANALYSIS_" + cleanDay + "_STEP4";
