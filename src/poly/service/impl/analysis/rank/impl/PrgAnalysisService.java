 ...
 * 퀴즈톡 진행률에 대한 기준 값
 * 
 * 퀴즈팩 조회의 기준값 : 최대 조회수와 평균 조회수의 중간 값
 * 
 * 조회수가 '퀴즈팩 조회의 기준값' 보다 큰 경우, 무조건 진행률은 100%(1)로 판단함
 * 
 * */
@Service("PrgAnalysisService")
public class PrgAnalysisService extends MongoDBComon implements IPrgAnalysisService, ICommCont {
...
//	final private double dataSize = 10000d;
	...
	// 오늘의 최대 퀴즈팩 풀이 진행률
	private double maxPrgForQuizPack = 0;

	// 오늘의 평균 퀴즈팩 풀이 진행률
	private double avgPrgForQuizPack = 0;

	// 오늘의 평균 퀴즈팩 풀이 기준 진행률
	private double stdPrgForQuizPack = 0;
	 ...
	 * 진행률 분석 1단계
	 * 
	 * 퀴즈팩별 진행률 분석 - QUIZLOG_STEP2_yyyyMMdd 데이터로부터 분석 - 1차 데이터분석 진행
	 */
	private int doAnalysisStep1() throws Exception {
		...
		try {
			...
			/*
			 * #############################################################################
			 * 퀴즈팩별 진행률 분석 결과 저장을 위한 컬렉션 생성 시작!
			 * #############################################################################
			 */

			// 퀴즈팩별 진행률 분석 결과 저장을 위한 컬렉션 이름
			String nColNm = "QUIZRANK_PRG_ANALYSIS_" + ContAnaysisStdDay + "_STEP1";
			...
			// 퀴즈팩별 진행률 분석 결과 저장을 위한 컬렉션 생성
			mongodb.createCollection(nColNm).createIndex(Indexes.ascending("qp_id"));

			/*
			 * #############################################################################
			 * 퀴즈팩별 진행률 분석 결과 저장을 위한 컬렉션 생성 끝!
			 * #############################################################################
			 */

			/*
			 * #############################################################################
			 * 퀴즈팩별 진행률 분석 결과 저장을 위한 컬렉션 데이터 구조 생성 시작!
			 * #############################################################################
			 */
			...
			// 데이터 쿼리 생성하기
			// 퀴즈팩의 문제푼 사용자가 30명 이상 퀴즈팩 정보만 가져오기
			List<? extends Bson> pipeline = Arrays.asList(
					new Document().append("$group",
							new Document().append("_id", new Document().append("qp_id", "$qp_id"))
									.append("AVG(user_quiz_cnt)", new Document().append("$avg", "$user_quiz_cnt"))
									.append("COUNT(qp_id)", new Document().append("$sum", 1))),
					new Document().append("$project",
							new Document().append("qp_id", "$_id.qp_id")
									.append("AVG(user_quiz_cnt)", "$AVG(user_quiz_cnt)")
									.append("COUNT(qp_id)", "$COUNT(qp_id)").append("_id", 0)),
					new Document().append("$match", new Document().append("COUNT(qp_id)",
							new Document().append("$gt", ICommCont.ContMinUserCnt))));
			...
			while (cursor.hasNext()) {
				...
				// 퀴즈팩별 문제푼 사용자의 수
				final int user_cnt = NumberUtil.getInt(current.get("COUNT(qp_id)"));

				// 퀴즈팩별 문제푼 퀴즈의 수
				final double quiz_cnt = NumberUtil.getDouble(current.get("AVG(user_quiz_cnt)"));

// 				log.info("qp_id : "+ qp_id);
// 				log.info("user_cnt : "+ user_cnt);
// 				log.info("quiz_cnt : "+ quiz_cnt);

				// 퀴프팩별 진행률 정보 저장하기
				doc = new Document();

				doc.append("qp_id", qp_id);
				doc.append("user_cnt", user_cnt);
				doc.append("quiz_cnt", quiz_cnt);
			...
			/*
			 * #############################################################################
			 * 퀴즈팩별 진행률 분석 결과 저장을 위한 컬렉션 데이터 구조 생성 결과 통계
			 * #############################################################################
			 */
			log.info("#################################################################");
			...
			/*
			 * #############################################################################
			 * 퀴즈팩별 진행률 분석 결과 저장을 위한 컬렉션 데이터 구조 생성 끝!
			 * #############################################################################
			 */
	 ...
	 * 진행률 분석 2단계
	 * 
	 * 퀴즈팩별 오늘 최대 풀이횟수 분석 - 1단계에서 생성된 QUIZ_PRG_STEP1__yyyyMMdd 데이터로부터 분석 - 2차 데이터분석
	 * 진행
	 */
	private int doAnalysisStep2() throws Exception {
		...
		try {
			...
			/*
			 * #############################################################################
			 * 퀴즈 진행률 구현을 위한 오늘의 최대 진행률 데이터가 저장되는 컬렉션 생성 시작!
			 * #############################################################################
			 */

			// 오늘의 퀴즈팩별 최대 진행율 데이터가 저장되는 컬렉션 이름
			String nColNm = "QUIZRANK_PRG_ANALYSIS_" + ContAnaysisStdDay + "_STEP2";
			...
			// 오늘의 퀴즈팩별 최대 진행율 데이터가 저장되는 컬렉션 생성
			mongodb.createCollection(nColNm).createIndex(Indexes.ascending("prg_date"));

			/*
			 * #############################################################################
			 * 퀴즈 진행률 구현을 위한 오늘의 최대 진행률 데이터가 저장되는 컬렉션 생성 끝!
			 * #############################################################################
			 */

			/*
			 * #############################################################################
			 * 오늘의 최대, 평균, 기준 진행률 값 생성하기 시작!
			 * #############################################################################
			 */

			MongoCollection<Document> rCol = mongodb
					.getCollection("QUIZRANK_PRG_ANALYSIS_" + ContAnaysisStdDay + "_STEP1");

			// 데이터 쿼리 생성하기
			List<? extends Bson> pipeline = Arrays.asList(
					new Document().append("$group",
							new Document().append("_id", new Document())
									.append("AVG(quiz_cnt)", new Document().append("$avg", "$quiz_cnt"))
									.append("MAX(quiz_cnt)", new Document().append("$max", "$quiz_cnt"))),
					new Document().append("$project", new Document().append("AVG(quiz_cnt)", "$AVG(quiz_cnt)")
							.append("MAX(quiz_cnt)", "$MAX(quiz_cnt)").append("_id", 0)));
			...
			if (cursor.hasNext()) {
				...
				// 오늘의 퀴즈팩별 최대 진행률
				this.maxPrgForQuizPack = NumberUtil.getDouble(current.get("MAX(quiz_cnt)"));

				// 오늘의 퀴즈팩별 평균 진행률
				this.avgPrgForQuizPack = NumberUtil.getDouble(current.get("AVG(quiz_cnt)"));

				// 오늘의 퀴즈팩별 기준 진행률
				this.stdPrgForQuizPack = Math
						.round(((this.avgPrgForQuizPack + this.maxPrgForQuizPack) / 2) * ContDataSize) / ContDataSize;

				Document doc = new Document();

				doc.append("prg_date", ContAnaysisStdDay);
				doc.append("max_rate", this.maxPrgForQuizPack);
				doc.append("avg_rate", this.avgPrgForQuizPack);
				doc.append("std_rate", this.stdPrgForQuizPack);
	 ...
	 * 진행률 분석 3단계
	 * 
	 * 퀴즈팩별 가중치 부여 - 1단계에서 생성된 QUIZ_PRG_STEP1__yyyyMMdd 데이터로부터 분석 - 3차 데이터분석 진행
	 */
	private int doAnalysisStep3() throws Exception {
		...
		try {
			...
			/*
			 * #############################################################################
			 * 가중치가 적용된 퀴즈팩별 진행률 데이터 분석 결과가 저장되는 컬렉션 생성 시작!
			 * #############################################################################
			 */

			// 가중치가 적용된 퀴즈팩별 진행률 데이터 분석 결과가 저장되는 컬렉션 이름
			String nColNm = "QUIZRANK_PRG_ANALYSIS_" + ContAnaysisStdDay + "_STEP3";
			...
			// 가중치가 적용된 퀴즈팩별 진행률 데이터 분석 결과가 저장되는 컬렉션 생성
			mongodb.createCollection(nColNm).createIndex(Indexes.ascending("qp_id"));

			/*
			 * #############################################################################
			 * 가중치가 적용된 퀴즈팩별 진행률 데이터 분석 결과가 저장되는 컬렉션 생성 끝!
			 * #############################################################################
			 */

			/*
			 * #############################################################################
			 * 퀴즈팩 전체 정답률을 추가하기 위한 퀴즈팩별 진행률 가중치 계산값이 저장되는 컬렉션 생성 데이터 구조 생성 시작 !
			 * #############################################################################
			 */
			...
			while (cursor.hasNext()) {
				...
				// 퀴즈팩별 진행률
				double avrQuizCnt = NumberUtil.getDouble(current.get("quiz_cnt"));
				doc.append("quiz_cnt", avrQuizCnt);

				// 전체 퀴즈팩 기준 진행률과 퀴즈팩별 진행률에 대한 가중치 계산
				double rateWeight = Math.round((avrQuizCnt / this.stdPrgForQuizPack) * ContDataSize) / ContDataSize;
				...
				doc.append("quiz_rate", rateWeight);
	 ...
	 * 진행률 분석 4단계
	 * 
	 * 최근 7일동안의 일자별 퀴즈팩들의 진행률 및 가중치 적용된 진행률 데이터 구조 생성
	 */
	private int doAnalysisStep4() throws Exception {
		...
		try {
			...
			/*
			 * #############################################################################
			 * 최근 7일동안의 일자별 퀴즈팩들의 진행률 및 가중치 적용된 진행률 데이터 구조를 저장할 컬렉션 생성 시작!
			 * #############################################################################
			 */

			// 최근 7일동안의 일자별 퀴즈팩들의 진행률 및 가중치 적용된 진행률 데이터 구조를 저장할 컬렉션 이름
			String nColNm = "QUIZRANK_PRG_ANALYSIS_" + ContAnaysisStdDay + "_STEP4";
			...
			// 최근 7일동안의 일자별 퀴즈팩들의 진행률 및 가중치 적용된 진행률 데이터 구조를 저장할 컬렉션 생성
			mongodb.createCollection(nColNm).createIndex(Indexes.ascending("qp_id"));

			/*
			 * #############################################################################
			 * 최근 7일동안의 일자별 퀴즈팩들의 진행률 및 가중치 적용된 진행률 데이터 구조를 저장할 컬렉션 생성 끝!
			 * #############################################################################
			 */

			/*
			 * #############################################################################
			 * 최근 7일동안의 일자별 퀴즈팩들의 진행률 및 가중치 적용된 진행률 데이터 구조 생성 시작 !
			 * #############################################################################
			 */
			...
			for (int i = 0; i < ContReadCntDay; i++) {
				...
				// 이전 조회수 저장 컬렉션 로드하기
				MongoCollection<Document> rCol = mongodb.getCollection("QUIZRANK_PRG_ANALYSIS_" + stdDay + "_STEP3");

				log.info("#################################################################");
				log.info("# QUIZRANK_PRG_ANALYSIS_" + stdDay + "_STEP3 Analysis Start!!");
				log.info("#################################################################");
				...
				while (cursor.hasNext()) {
					...
					double prgRate = NumberUtil.getDouble(current.get("quiz_cnt")); // 퀴즈푼 숫자
					double rateWeight = NumberUtil.getDouble(current.get("quiz_rate")); // 가중치가 적용된 진행률

					if (i == 0) {
						doc.append("stdRate1", prgRate);
						doc.append("rateWeight1", rateWeight);

					} else if (i == 1) {
						doc.append("stdRate2", prgRate);
						doc.append("rateWeight2", rateWeight);

					} else if (i == 2) {
						doc.append("stdRate3", prgRate);
						doc.append("rateWeight3", rateWeight);

					} else if (i == 3) {
						doc.append("stdRate4", prgRate);
						doc.append("rateWeight4", rateWeight);

					} else if (i == 4) {
						doc.append("stdRate5", prgRate);
						doc.append("rateWeight5", rateWeight);

					} else if (i == 5) {
						doc.append("stdRate6", prgRate);
						doc.append("rateWeight6", rateWeight);

					} else if (i == 6) {
						doc.append("stdRate7", prgRate);
				...
				log.info("#################################################################");
				log.info("# Save QUIZRANK_PRG_ANALYSIS_" + ContAnaysisStdDay + "_STEP4 Analysis Result!!");
				log.info("# " + nColNm + " insert Doc Count : " + mongodb.getCollection(nColNm).countDocuments());
				log.info("#################################################################");
			...
			/*
			 * #############################################################################
			 * 최근 7일동안의 일자별 퀴즈팩들의 진행률 및 가중치 적용된 진행률 데이터 구조 생성 결과 통계
			 * #############################################################################
			 */
			log.info("#################################################################");
			...
			/*
			 * #############################################################################
			 * 최근 7일동안의 일자별 퀴즈팩들의 진행률 및 가중치 적용된 진행률 데이터 구조 생성 끝!!
			 * #############################################################################
			 */
	 ...
	 * 정답률 분석 5단계
	 * 
	 * 퀴즈팩별 가중치가 적용된 진행률 최종 결과를 RDBMS에 저장
	 */
	private int doAnalysisStep5() throws Exception {
		...
		try {
			...
			/*
			 * #############################################################################
			 * 퀴즈팩별 가중치가 적용된 진행률 최종 결과가 저장되는 컬렉션 생성 시작!
			 * #############################################################################
			 */

			// 퀴즈팩별 가중치가 적용된 진행률 최종 결과가 저장되는 컬렉션 이름
			String nColNm = "QUIZRANK_PRG_ANALYSIS_" + ContAnaysisStdDay + "_STEP5";
			...
			// 퀴즈팩별 가중치가 적용된 진행률 최종 결과가 저장되는 컬렉션 생성
			mongodb.createCollection(nColNm).createIndex(Indexes.ascending("qp_id"));

			/*
			 * #############################################################################
			 * 퀴즈팩별 가중치가 적용된 진행률 최종 결과가 저장되는 컬렉션 생성 끝!
			 * #############################################################################
			 */

			/*
			 * #############################################################################
			 * 퀴즈 진행률 구현을 위한 일자별 데이터가 저장되는 컬렉션 생성 데이터 구조 생성 시작 !
			 * #############################################################################
			 */

			MongoCollection<Document> rCol = mongodb
					.getCollection("QUIZRANK_PRG_ANALYSIS_" + ContAnaysisStdDay + "_STEP4");
			...
			/*
			 * #############################################################################
			 * 퀴즈 진행률 구현을 위한 일자별 데이터가 저장되는 컬렉션 데이터 구조 생성 끝 !
			 * #############################################################################
			 */
	...
	private int doCleanData() throws Exception {
		...
		try {
			...
			// 1단계 분석 삭제
			dropColNm = "QUIZRANK_PRG_ANALYSIS_" + cleanDay + "_STEP1";
			...
			// 2단계 분석 삭제
			dropColNm = "QUIZRANK_PRG_ANALYSIS_" + cleanDay + "_STEP2";
			...
			// 4단계 분석 삭제
			dropColNm = "QUIZRANK_PRG_ANALYSIS_" + cleanDay + "_STEP4";
			...
			// 5단계 분석 삭제
			dropColNm = "QUIZRANK_PRG_ANALYSIS_" + cleanDay + "_STEP5";
