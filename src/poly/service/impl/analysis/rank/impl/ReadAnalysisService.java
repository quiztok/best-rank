...
@Service("ReadAnalysisService")
public class ReadAnalysisService extends MongoDBComon implements IReadAnalysisService, ICommCont {
	...
	private int doAnalysisStep1() throws Exception {
		...
		try {
			...
			String nColNm = "QUIZRANK_READ_ANALYSIS_" + ContAnaysisStdDay + "_STEP1";
			...
			List<? extends Bson> pipeline = Arrays.asList(
					new Document().append("$group",
							new Document().append("_id", new Document())
									.append("MAX(click_cnt)", new Document().append("$max", "$click_cnt"))
									.append("AVG(click_cnt)", new Document().append("$avg", "$click_cnt"))),
					new Document().append("$project", new Document().append("max_cnt", "$MAX(click_cnt)")
							.append("avg_cnt", "$AVG(click_cnt)").append("_id", 0)));
	...
	private int doAnalysisStep2() throws Exception {
		...
		try {
			...
			// 컬렉션 이름
			String nColNm = "QUIZRANK_READ_ANALYSIS_" + ContAnaysisStdDay + "_STEP2";
			...
			while (cursor.hasNext()) {
				...
				// 퀴즈팩별 중복되지 않는 조회수수
				long cnt = (long) current.get("click_cnt");
				doc.append("cnt", cnt);

				// 전체 퀴즈팩 기준 조회수율과 퀴즈팩별 조회수수에 대한 가중치 계산
			...
			/*
			 * #############################################################################
			 * 퀴즈팩 전체 정답률을 추가하기 위한 퀴즈팩별 조회수수 가중치 계산값이 저장되는 컬렉션 데이터 생성 결과 통계
			 * #############################################################################
			 */
			log.info("#################################################################");
	 ...
	 * 분석 3단계
	 * 
	 * 최근 7일동안의 일자별 퀴즈팩들의 조회수 및 가중치 적용된 조회수률 데이터 구조 생성
	 */
	private int doAnalysisStep3() throws Exception {
		...
		try {
			...
			String nColNm = "QUIZRANK_READ_ANALYSIS_" + ContAnaysisStdDay + "_STEP3";
			...
			/*
			 * #############################################################################
			 * 최근 7일동안의 일자별 퀴즈팩들의 조회수 및 가중치 적용된 조회수 데이터 구조 생성 시작!!
			 * #############################################################################
			 */
			...
			for (int i = 0; i < ContReadCntDay; i++) {
				...
				// 이전 조회수수 저장 컬렉션 로드하기
				MongoCollection<Document> rCol = mongodb.getCollection("QUIZRANK_READ_ANALYSIS_" + stdDay + "_STEP2");

				log.info("#################################################################");
				log.info("# Read QUIZRANK_READ_ANALYSIS_" + stdDay + "_STEP2 Analysis Start!!");
				log.info("#################################################################");
				...
				log.info("#################################################################");
				log.info("# Save QUIZRANK_READ_ANALYSIS_" + stdDay + "_STEP3 Analysis Result!!");
				log.info("# " + nColNm + " insert Doc Count : " + mongodb.getCollection(nColNm).countDocuments());
				log.info("#################################################################");
			...
			/*
			 * #############################################################################
			 * 최근 7일동안의 일자별 퀴즈팩들의 조회수 및 가중치 적용된 조회수 데이터 구조 생성 끝!!
			 * #############################################################################
			 */
	...
	private int doAnalysisStep4() throws Exception {
		...
		try {
			...
			String nColNm = "QUIZRANK_READ_ANALYSIS_" + ContAnaysisStdDay + "_STEP4";
			...
			MongoCollection<Document> rCol = mongodb
					.getCollection("QUIZRANK_READ_ANALYSIS_" + ContAnaysisStdDay + "_STEP3");
			...
			/*
			 * #############################################################################
			 * 퀴즈 조회수율 구현을 위한 일자별 데이터가 저장되는 컬렉션 데이터 구조 생성 끝 !
			 * #############################################################################
			 */
	...
	private int doCleanData() throws Exception {
		...
		try {
			...
			// 1단계 분석 삭제
			dropColNm = "QUIZRANK_READ_ANALYSIS_" + cleanDay + "_STEP1";
			...
			// 3단계 분석 삭제
			dropColNm = "QUIZRANK_READ_ANALYSIS_" + cleanDay + "_STEP3";
			...
			// 4단계 분석 삭제
			dropColNm = "QUIZRANK_READ_ANALYSIS_" + cleanDay + "_STEP4";
